...
acquisition has not been wired into the hostile-input consumer path, so that
stage is `SKIP` and every dependent stage is blocked. The ingestion replay
workflow verifies that this cannot be reported as a successful zero-candidate
search.
